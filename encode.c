...
void print_usage(){
  fprintf(stderr, "Usage: encode [-s speed] [-d direction] [-m mode] temperature\n
	Speed = AUTO, 1, 3, 5\n
	Direction = 0: UP, 1, 2, 3, 4:DOWN, 5: SWING\n
	Mode= COOL, HEAT\n");
  exit(0);
}
...
int main(int argc, char** argv){
    ...
    while ((opt = getopt(argc, argv, "s:d:m:")) != -1) {
		...
		command[8] = command[8] | (SPEED_AUTO << 5); 
	    else if (!strcmp(optarg,"1"))
		command[8] = command[8] | (SPEED1 << 5);
	    else if (!strcmp(optarg,"3"))
		command[8] = command[8] | (SPEED3 << 5);
	    else if (!strcmp(optarg,"5"))
		command[8] = command[8] | (SPEED5 << 5);
	    else{
